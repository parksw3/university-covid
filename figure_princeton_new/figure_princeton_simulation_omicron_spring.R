...
load("../simulation_omicron_spring/simulation_omicron_spring_noswitch.rda")
load("../simulation_omicron_spring/simulation_omicron_spring.rda")
load("../simulation_omicron_spring/simulation_omicron_spring_increase_20.rda")
load("../simulation_omicron_spring/simulation_omicron_spring_increase_50.rda")
load("../simulation_omicron_spring/simulation_omicron_spring_increase_100.rda")

simall_spring_2021_noswitch <- lapply(simulation_omicron_spring_noswitch, function(x){
  lapply(x, function(y) {
    data.frame(
      date=y$datevec,
...
simall_spring_2021 <- lapply(simulation_omicron_spring, function(x){
  lapply(x, function(y) {
    data.frame(
      date=y$datevec,
      time=1:length(y$incidencevec),
...
simall_spring_2021_increase_20 <- lapply(simulation_omicron_spring_increase_20, function(x){
  lapply(x, function(y) {
    data.frame(
      date=y$datevec,
      time=1:length(y$incidencevec),
...
simall_spring_2021_increase_50 <- lapply(simulation_omicron_spring_increase_50, function(x){
  lapply(x, function(y) {
    data.frame(
      date=y$datevec,
      time=1:length(y$incidencevec),
...
simall_spring_2021_increase_100 <- lapply(simulation_omicron_spring_increase_100, function(x){
  lapply(x, function(y) {
    data.frame(
      date=y$datevec,
      time=1:length(y$incidencevec),
...
simall_spring_all <- bind_rows(
  simall_spring_2021_noswitch %>% mutate(type="No~switching"),
  simall_spring_2021 %>% mutate(type="Weekly~testing"),
  simall_spring_2021_increase_20 %>% mutate(type="Weekly~testing~\n~20~'%'~increase~'in'~reproduction~number"),
...
ggsave("figure_princeton_simulation_omicron_spring.pdf", g1, width=12, height=8)
