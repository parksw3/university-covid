library(dplyr)
source("../R/simulate_internal_omicron_switch.R")
source("../R/simulate_omicron_switch.R")
...
simulation_omicron_spring_noswitch <- vector('list')
...
for (j in 1:nrow(paramvec)) {
  ...
  for (i in 1:nsim) {
    ## 5000 undergrad
    print(paste0(j, ", ", i))
    ss <- simulate_omicron_switch(param=drawpar(preport=1, R0=paramvec[j, 1]),
                           N=5000,
                           I=0.14,
                           R=100/5000,
  ...
  simulation_omicron_spring_noswitch[[j]] <- tmplist
}

save("simulation_omicron_spring_noswitch", file="simulation_omicron_spring_noswitch.rda")
