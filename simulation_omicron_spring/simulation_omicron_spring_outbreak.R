library(dplyr)
source("../R/simulate_internal_omicron_switch_outbreak.R")
source("../R/simulate_omicron_switch_outbreak.R")
...
simulation_omicron_spring_outbreak <- vector('list')

theta <- 0.15 * mercer_spring2021$cases * 5000/13000

R0vec <- c(2, 4, 6)
outbreaksize <- c(100, 150, 200, 250, 300)

paramvec <- expand.grid(R0vec, outbreaksize)
...
for (j in 1:nrow(paramvec)) {
  ...
  for (i in 1:nsim) {
    ## 5000 undergrad
    print(paste0(j, ", ", i))
    ss <- simulate_omicron_switch_outbreak(param=drawpar(preport=1, R0=paramvec[j, 1]),
                           N=5000,
                           I=0.14,
                           R=100/5000,
    ...
    ss2$R0 <- paramvec[j,1]
    ss2$outbreaksize <- paramvec[j,2] 
    
    tmplist[[i]] <- ss2
  }
  
  simulation_omicron_spring_outbreak[[j]] <- tmplist
}

save("simulation_omicron_spring_outbreak", file="simulation_omicron_spring_outbreak.rda")
