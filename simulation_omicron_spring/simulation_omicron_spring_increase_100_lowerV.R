...
simulation_omicron_spring_increase_100_lowerV <- vector('list')
...
for (j in 1:nrow(paramvec)) {
  ...
  for (i in 1:nsim) {
    ## 5000 undergrad
    print(paste0(j, ", ", i))
    ss <- simulate_omicron_switch2(param=drawpar(preport=1, R0=paramvec[j, 1]),
                           N=5000,
                           I=0.14,
                           R=100/5000,
                           ...
                           veff_threedose_inf = 0.3 * 0.2/0.9,
                           vaccine_delay=7,
                           iperiod=5,
                           ntest = 8000,
                           asymp_testing=TRUE,
  ...
  simulation_omicron_spring_increase_100_lowerV[[j]] <- tmplist
}

save("simulation_omicron_spring_increase_100_lowerV", file="simulation_omicron_spring_increase_100_lowerV.rda")
