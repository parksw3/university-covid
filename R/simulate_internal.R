...
simulate_internal <- function(state,
                              vacc,
                              veff,
                              return,
                              returnfrac=1/7,
    ...
    if (t >= 2) {
      ...
      for (w in whichinfect) {
        ...
        if (ninfect > 0) {
          ...
          ## vaccination...
          ww_group <- ww_group[rbinom(length(ww_group), size=1, prob=(1-veff * vacc[ww_group]))==1]
...
infectfun <- function(x,
                      ...
                      .transmission,
                      .state) {
  trate <- .transmission[match(x, .state)] * (1-vstate * veff)
