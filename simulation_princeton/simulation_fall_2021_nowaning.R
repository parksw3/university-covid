...
for (j in 1:nrow(paramvec)) {
  ...
  for (i in 1:nsim) {
    ...
    print(paste0(j, ", ", i))
    ss <- simulate_omicron(param=drawpar(preport=1, R0=paramvec[j, 1], delta=0),
                   N=13000,
                   I=0.005,
                   vprop=0.98,
                   boostprop=0,
  ...
  simulation_fall_2021_nowaning[[j]] <- tmplist
}

save("simulation_fall_2021_nowaning", file="simulation_fall_2021_nowaning.rda")
